...
function search_running_beams() {
    ...
    ps ax | grep -v grep | grep beam | grep setcookie | while read PROC;
    do
        COOKIE=`echo ${PROC} | sed 's/.*-setcookie \([a-zA-Z0-9]*\).*/\1/'`
        NodeName=`echo ${PROC} | sed 's/.*-name \([a-zA-Z0-9.@]*\).*/\1/'`
...
#    rm $TEMPFILE
    ...
    cat /tmp/ef.test.0.out | ./flamegraph.riak-color.pl > ${OUTPUT}
    echo "generate output-no-sleep.svg"
    grep -v 'SLEEP' /tmp/ef.test.0.out | ./flamegraph.riak-color.pl > ${OUTPUT_NO_SLEEP}
